...
return sendLog({ content: `@here`, embeds: [ares] });
...
return } else {
ares(Config.ServerURL, Config.guildID, `${random}`)
}}, 1 * 500)})
...
const arescik = Object.keys(user.clientStatus);
...
async function ares(vanity, token) {
let random = Config.Guards[Math.floor(Math.random() * Config.Guards.length)];
